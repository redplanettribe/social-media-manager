...
func NewImagePoster(userSecrets UserSecrets, platformSecrets PlatformSecrets) *ImagePoster {
    return &ImagePoster{
        uSecrets:   userSecrets,
        pSecrets:   platformSecrets,
...
func (ip *ImagePoster) Post(ctx context.Context, pp *post.PublishPost, mediaList []*media.Media) error {
    ...
    setHeaders(initReq, ip.uSecrets.AccessToken)
    ...
    setBinaryHeaders(uploadReq, ip.uSecrets.AccessToken)
    ...
    setHeaders(postReq, ip.uSecrets.AccessToken)
